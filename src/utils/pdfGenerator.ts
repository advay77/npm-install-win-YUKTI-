...
export const generatePDFReport = async (feedbackData: any, interviewInfo: any) => {
  ...
  try {
    ...
    // Transcripts Section
    const transcripts = feedbackData?.metadata?.transcripts || [];
    if (transcripts && transcripts.length > 0) {
      checkAndAddPage(60);
      yPosition += 10;
      addColoredText('Interview Transcript', 20, yPosition, primaryColor, 16, true);
      yPosition += 10;

      transcripts.forEach((msg: any) => {
        const isAI = msg.type === "assistant";
        const roleText = isAI ? "Interviewer:" : "You:";
        addColoredText(roleText, 20, yPosition, isAI ? [124, 58, 237] : [59, 130, 246], 11, true); // Purple for AI, Blue for User

        const msgLines = pdf.splitTextToSize(msg.content, pageWidth - 45);
        // Start text slightly right of role
